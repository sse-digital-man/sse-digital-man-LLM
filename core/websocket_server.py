...
class MyServer:
    ...
    def __connect(self):
        ...
        if self.__server:
            ue_utils.log(1, 'server already exist')
            return
    ...
    def add_cmd(self, content):
        ...
        ue_utils.log(1, '命令 {}'.format(content))
    ...
    async def __handler(self, websocket, path):
        self.isConnect = True
        ue_utils.log(1, "websocket连接上:{}".format(self.__port))
        ...
        for task in self.__pending:
            task.cancel()
            self.isConnect = False
            ue_utils.log(1, "websocket连接断开:{}".format(self.__port))
            self.on_close_handler()

    async def __consumer(self, message):
        self.on_receive_handler(message)
    ...
    @abstractmethod
    def on_receive_handler(self, message):
        pass
